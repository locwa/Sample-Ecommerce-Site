    ...
    if (isActive) {
        return (
            <div className='border-b border-[#5ECE7B] h-12 px-2 flex items-center '>
                <NavLink to={ path } className='font-bold text-[#5ECE7B]' data-testid="active-category-link">{name}</NavLink>
            </div>
        )
    } else {
        return (
            <NavLink to={ path } className='h-12 px-2 flex items-center' data-testid="category-link">{name}</NavLink>
        )
    }
}
    ...
    return (
        <nav className='sticky top-0 z-50 bg-white w-full flex justify-between items-center px-10 py-2 bg-white z-2' onClick={ () => isCartOpen && closeCart()}>
            <div className='flex gap-10' >
                <NavlinkComponent
                    path='/all'
                    isActive={(category == undefined || category == "all")}
                    name='ALL'
                />
                <NavlinkComponent
                    path='/clothes'
